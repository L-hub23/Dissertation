...
new_affiliations <- rbind(new_affiliations, data.frame(Element = c("call_of_duty:_black_ops_cold_war", "borat_subsequent_moviefilm", "joe_exotic", "stephen_colbert", "billie_eilish", "ken_burns", "tom_brady", "john_wane_gacy", "ken_jennings", "beyoncé", "stephen_king"), Category = "Other"))
...
for (original_name in names(page_views_republican_jan)) {
  # Retrieve the value associated with the original name
  page_views_jan_rep <- page_views_republican[[original_name]]
...
for (original_name in names(page_views_democrat_jan)) {
  # Retrieve the value associated with the original name
  page_views_jan_dem <- page_views_democrat[[original_name]]
...
matched_names_rep_jan <- network_names_dec[network_names_jan %in% names(updated_rep_pv_list_jan)]
...
# order by descending page views
page_views_republican_filtered_jan <- page_views_republican_filtered_jan[order(unlist(page_views_republican_filtered_jan), decreasing = TRUE)]

page_views_democrat_filtered_jan <- page_views_democrat_filtered_jan[order(unlist(page_views_democrat_filtered_jan), decreasing = TRUE)]
...
for (original_name in names(page_views_republican_feb)) {
  # Retrieve the value associated with the original name
  page_views_feb <- page_views_republican[[original_name]]
...
for (original_name in names(page_views_democrat_feb)) {
  # Retrieve the value associated with the original name
  page_views_feb <- page_views_democrat[[original_name]]
...
matched_names_rep_feb <- network_names_dec[network_names_feb %in% names(updated_rep_pv_list_feb)]
...
# order by descending page views
page_views_republican_filtered_feb <- page_views_republican_filtered_feb[order(unlist(page_views_republican_filtered_feb), decreasing = TRUE)]

page_views_democrat_filtered_feb <- page_views_democrat_filtered_feb[order(unlist(page_views_democrat_filtered_feb), decreasing = TRUE)]
...
analyze_party_walk <- function(graph, party, vertices, steps = 10, iterations = 1) {
  ...
  return(list(
    steps = mean_steps, n_part_shift = n_part_shift, n_trapped = n_trapped, mean_steps_trapped = mean_trapped_steps, n_republican = n_republican, n_democrat = n_democrat, n_politics = n_politics, n_other = n_other))
}
...
# get a random sample of 100 pages
set.seed(42)
# get all democrat pages of the network
page_names <- V(net_clicknov20)$name[V(net_clicknov20)$party == "Democrat"]
page_names_d <- sample(page_names, 100)

results_nov20 <- analyze_party_walk(net_clicknov20, "Democrat", page_names_d, steps = 5, iterations = 10)

results_dec20 <- analyze_party_walk(net_clickdec20, "Democrat", page_names_d, steps = 10, iterations = 10)
```

...
clean_graph <- function(graph) {
  ...
  #print(sum(degree(cleaned_graph) == 0))
...
# count number of republican and democrat nodes
sum(V(net_clicksep20)$party == "Republican")
sum(V(net_clicksep20)$party == "Democrat")

# check unique values for party
unique(V(net_clicksep20)$party)
```
...
# update graphs with the affiliation of pages appearing as both republican and democrat
# as well as nodes from the top100 pages that are deemed irrelevant
...
number_pages_republican <- data.frame(
  ...
  Pages = c(analyse_nov19$num_republican, analyse_dec19$num_republican, analyse_jan20$num_republican, analyse_feb20$num_republican, analyse_mar20$num_republican, analyse_apr20$num_republican, analyse_may20$num_republican, analyse_jun20$num_republican, analyse_jul20$num_republican, analyse_aug20$num_republican, analyse_sep20$num_republican, analyse_oct20$num_republican, analyse_nov20$num_republican, analyse_dec20$num_republican, analyse_jan21$num_republican, analyse_feb21$num_republican, analyse_mar21$num_republican, analyse_apr21$num_republican, analyse_may21$num_republican, analyse_jun21$num_republican, analyse_jul21$num_republican, analyse_aug21$num_republican, analyse_sep21$num_republican, analyse_oct21$num_republican, analyse_nov21$num_republican))
...
number_pages_democrat <- data.frame(
  Month = c("November 2019", "December 2019", "January 2020", "February 2020", "March 2020", "April 2020", "May 2020", "June 2020", "July 2020", "August 2020", "September 2020", "October 2020", "November 2020", "December 2020", "January 2021", "February 2021", "March 2021", "April 2021", "May 2021", "June 2021", "July 2021", "August 2021", "September 2021", "October 2021", "November 2021"),
  Pages = c(analyse_nov19$num_democrat, analyse_dec19$num_democrat, analyse_jan20$num_democrat, analyse_feb20$num_democrat, analyse_mar20$num_democrat, analyse_apr20$num_democrat, analyse_may20$num_democrat, analyse_jun20$num_democrat, analyse_jul20$num_democrat, analyse_aug20$num_democrat, analyse_sep20$num_democrat, analyse_oct20$num_democrat, analyse_nov20$num_democrat, analyse_dec20$num_democrat, analyse_jan21$num_democrat, analyse_feb21$num_democrat, analyse_mar21$num_democrat, analyse_apr21$num_democrat, analyse_may21$num_democrat, analyse_jun21$num_democrat, analyse_jul21$num_democrat, analyse_aug21$num_democrat, analyse_sep21$num_democrat, analyse_oct21$num_democrat, analyse_nov21$num_democrat))
...
total_rep_dem_pages_table <- kable(total_rep_dem_pages, 
                                   caption = "<div style='text-align: center;'><span style='color: black; font-weight: bold;'>Table 1. Total Pages included Hyperlink Networks by Month</span></div>",
                                   col.names = c("Month", "Republican Pages", "Democrat Pages"),
                                   format = "html") %>%
  kable_styling(full_width = FALSE, font_size = 12) %>%
  row_spec(0, bold = TRUE, font_size = 14, color = "black", background = "#f2f2f2") %>%
  row_spec(1:nrow(total_rep_dem_pages), font_size = 12) %>%
  column_spec(1, width = "2in") %>%
  column_spec(2, width = "2in") %>%
  # Use CSS to apply font styling
  kable_styling(bootstrap_options = c("striped", "hover"), 
                font_size = 12, 
                position = "center", 
                fixed_thead = TRUE) %>%
  # Ensure the HTML table renders correctly in the environment
  htmltools::browsable()

# Print the table
total_rep_dem_pages_table
```
...
# Apply the function to each matrix and combine the results into a dataframe
...
ggplot(results_long, aes(x = Month, y = Count, color = Interaction, group = Interaction)) +
  geom_line() +
  geom_point(size = 1) +
  geom_vline(xintercept = 13, linetype = "dashed", color = "forestgreen", alpha = 0.5) +
  geom_vline(xintercept = 15, linetype = "dashed", color = "forestgreen", alpha = 0.5) +
  annotate("text", x = 13, y = 0.1, label = "Election", vjust = 0.5, color = "darkgreen", family = "Times New Roman", size = 4) +
  annotate("text", x = 15, y = 0.1, label = "Insurgence", vjust = -2, color = "darkgreen", family = "Times New Roman", size = 4) +
  labs(title = "Figure 1. Count of Edges included in Wikipedia hyperlink networks over time",
       x = "Month",
       y = "Edge Count", 
       color = "Link Type") +
...
calculate_modularity <- function(network, party_levels = c("Democrat", "Republican")) {
  ...
  mod <- modularity(network, membership, weights = E(network)$weight)
  return(mod)
}
...
ggplot(modularity_scores, aes(x = Month, y = Modularity)) +
  ...
  geom_hline(yintercept = average_modularity, linetype = "solid", color = "darkorange2", alpha = 0.6) + 
  geom_vline(xintercept = 13, linetype = "dashed", color = "forestgreen", alpha = 0.4) +
  geom_vline(xintercept = 15, linetype = "dashed", color = "forestgreen", alpha = 0.4) +
  annotate("text", x = 13, y = 0.1, label = "Election", vjust = 0, color = "darkgreen", family = "Times New Roman", size = 5) +
  annotate("text", x = 15, y = 0.1, label = "Insurgence", vjust = -2, color = "darkgreen", family = "Times New Roman", size = 5) +
  annotate("text", x = Inf, y = average_modularity, label = "Average Modularity", vjust = 1.5, hjust = 1, color = "darkorange2", family = "Times New Roman", size = 4) +
  labs(x = "Month", y = "Modularity", title = "Figure 2. Modularity Scores from November 2019 to November 2021") +
  theme_minimal() +  # Minimal theme
  theme(
    axis.text.x = element_text(size = 10, angle = 45, hjust = 1, family = "Times New Roman"),
    axis.text.y = element_text(size = 10, family = "Times New Roman"),
    axis.title.x = element_text(size = 15, family = "Times New Roman"),
    axis.title.y = element_text(size = 15, family = "Times New Roman"),
    plot.title = element_text(size = 18, family = "Times New Roman", hjust = 0.5),
...
calculate_spearman_correlation <- function(graph, community) {
  ...
  cross_boundary_connections <- function(graph, community) {
    sapply(V(graph), function(node) {
      ...
      sum(E(graph)$weight[edge_ids][community[neighbors] != community[node]])
    })
  }
  ...
  # Function to calculate polarization for a specific party
  calculate_party_polarization <- function(party) {
    party_nodes <- which(party_labels == party)
    if (length(party_nodes) < 2) {
      return(NA) # Not enough nodes to calculate correlation
    }
    
    party_degrees <- degrees[party_nodes]
    party_db <- db[party_nodes]
    
    party_rank_r <- rank(-party_degrees)
    party_rank_rb <- rank(-party_db)
    
    cor(party_rank_r, party_rank_rb, method = "spearman")
  }
...
# calculate correlation between nodes ranked by weighted degrees and normal degrees
  cross_boundary_connections_weighted <- function(graph, community) {
    sapply(V(graph), function(node) {
      neighbors <- neighbors(graph, node)
      edge_ids <- E(graph)[.from(node) & .to(neighbors)]
      # Sum the instances where neighboring nodes are in different communities
  ...
  cross_boundary_connections <- function(graph, community) {
    sapply(V(graph), function(node) {
      ...
      edge_ids <- E(graph)[.from(node) & .to(neighbors)]
      # Sum the instances where neighboring nodes are in different communities
      sum(community[neighbors] != community[node])
    })
  }

rank_cbw <- rank(-cross_boundary_connections_weighted(net_clicknov20_filtered, membership_nov20))
rank_cb <- rank(-cross_boundary_connections(net_clicknov20_filtered, membership_nov20))
rho <- cor(rank_d, rank_wd, method = "spearman")
...
```{r}
adjust_weights <- function(graph, target_strength, max_iter = 1000, tol = 1e-3) {
  current_strength <- strength(graph, mode = "out", weights = E(graph)$weight)
  iter <- 0
  while (iter < max_iter && max(abs(current_strength - target_strength)) > tol) {
    for (i in seq_along(V(graph))) {
      neighbors <- neighbors(graph, i)
      if (length(neighbors) == 0) next
      current_sum <- sum(E(graph)[.from(i)]$weight)
      target_sum <- target_strength[i]
      if (current_sum == 0) next
      adjustment_ratio <- target_sum / current_sum
      E(graph)[.from(i)]$weight <- E(graph)[.from(i)]$weight * adjustment_ratio
    }
    current_strength <- strength(graph, mode = "out", weights = E(graph)$weight)
    iter <- iter + 1
  }
...
random_net_nov19 <- adjust_weights(random_net_nov19, target_strength = strength(net_clicknov19_filtered, mode = "out"))
```
...
assign_attributes_and_calculate_correlation <- function(random_net, original_net, correlation = TRUE) {
  ...
  if (correlation == TRUE) {
   output <- calculate_spearman_correlation(random_net, random_membership) 
   return(output)
  }
}
...
ggplot(boundary_polarisation_scores, aes(x = Month)) +
  ...
  geom_vline(xintercept = 13, linetype = "dashed", color = "forestgreen", alpha = 0.4) +
  geom_vline(xintercept = 15, linetype = "dashed", color = "forestgreen", alpha = 0.4) +
  annotate("text", x = 13, y = 0.1, label = "Election", vjust = -1, color = "darkgreen", family = "Times New Roman", size = 5) +
  annotate("text", x = 15, y = 0.1, label = "Insurgence", vjust = -5, color = "darkgreen", family = "Times New Roman", size = 5) +
  labs(x = "Month", y = "Spearman's Rho", title = "Figure 3. Boundary Polarisation from November 2019 to February 2021", color = "Network type") +
  ...
  theme(
    ...
    axis.title.x = element_text(size = 15, family = "Times New Roman"),
    axis.title.y = element_text(size = 15, family = "Times New Roman"),
    plot.title = element_text(size = 18, hjust = 0.5, family = "Times New Roman"),
    legend.title = element_text(size = 15, family = "Times New Roman"),
    legend.text = element_text(size = 10, family = "Times New Roman")
  ) +
...
load("data/click_networks_rw.RData")
```
...
pages_to_filter <- c(
  "call_of_duty:_black_ops_cold_war", "borat_subsequent_moviefilm", "joe_exotic", "stephen_colbert", "billie_eilish", "ken_burns", "tom_brady", "john_wane_gacy", "ken_jennings", "beyoncé", "stephen_king", "jennifer_garner", "gilian_anderson", "jodie_foster", "caitlyn_jenner", "anthony_fauci", "space_shuttle_columbia_disaster", "taxi_driver", "buzz_aldrin", "ted_bundy", "wyatt_earp", "gadsden_flag", "vermin_supreme", "alec_baldwin", "jennifer_garner", "ku_klux_klan", "stephen_king", "columbine_high_school_massacre", "jon_bon_jovi", "monica_lewinsky", "guantanamo_bay_detention_camp", "warren_buffett", "2012_benghazi_attack", "jefferson_davis", "chicago_seven", "aretha_franklin", "ken_jennings", "henry_fonda", "2004_united_states_presidential_election", "timeline_of_russian_interference_in_the_2016_united_states_elections", "1984_united_states_presidential_election", "1988_united_states_presidential_election", "1980_united_states_presidential_election", "1976_united_states_presidential_election", "1964_united_states_presidential_election", "faithless_electors_in_the_2016_united_states_presidential_election", "1860_united_states_presidential_election", "1864_united_states_presidential_election", "1972_united_states_presidential_election", "1936_united_states_presidential_election", "1944_united_states_presidential_election", "1940_united_states_presidential_election", "1932_united_states_presidential_election", "1928_united_states_presidential_election", "1920_united_states_presidential_election", "iraq_war", "gulf_war", "reform_party_of_the_united_states_of_america", "constitution_party_(united_states)", "seth_macfarlane", "rocky_de_la_fuente", "billy_ray_cyrus", "deepwater_horizon_oil_spill", "killing_of_osama_bin_laden", "john_wayne_gacy" 
)

pages_to_filter_rep <- c(
  "call_of_duty:_black_ops_cold_war", "borat_subsequent_moviefilm", "joe_exotic", "stephen_colbert", "billie_eilish", "ken_burns", "tom_brady", "john_wane_gacy", "ken_jennings", "beyoncé", "stephen_king", "jennifer_garner", "gilian_anderson", "jodie_foster", "caitlyn_jenner", "anthony_fauci", "space_shuttle_columbia_disaster", "taxi_driver", "buzz_aldrin", "ted_bundy", "wyatt_earp", "gadsden_flag", "vermin_supreme", "alec_baldwin", "jennifer_garner", "ku_klux_klan", "stephen_king", "columbine_high_school_massacre", "jon_bon_jovi", "monica_lewinsky", "guantanamo_bay_detention_camp", "warren_buffett", "2012_benghazi_attack", "jefferson_davis", "chicago_seven", "aretha_franklin", "ken_jennings", "henry_fonda", "2004_united_states_presidential_election", "timeline_of_russian_interference_in_the_2016_united_states_elections", "1984_united_states_presidential_election", "1988_united_states_presidential_election", "1980_united_states_presidential_election", "1976_united_states_presidential_election", "1964_united_states_presidential_election", "faithless_electors_in_the_2016_united_states_presidential_election", "1860_united_states_presidential_election", "1864_united_states_presidential_election", "1972_united_states_presidential_election", "1936_united_states_presidential_election", "1944_united_states_presidential_election", "1940_united_states_presidential_election", "1932_united_states_presidential_election", "1928_united_states_presidential_election", "1920_united_states_presidential_election", "iraq_war", "gulf_war", "hillary_clinton", "reform_party_of_the_united_states_of_america", "constitution_party_(united_states)", "space_shuttle_columbia_disaster"
)
```
...
matched_names_rep_sep <- network_names_sep[network_names_sep %in% names(updated_rep_pv_list_sep)]
page_views_republican_filtered_sep <- updated_rep_pv_list_sep[matched_names_rep_sep]

matched_names_dem_sep <- network_names_sep[network_names_sep %in% names(updated_dem_pv_list_sep)]
page_views_democrat_filtered_sep <- updated_dem_pv_list_sep[matched_names_dem_sep]

# order by descending page views
page_views_republican_filtered_sep <- page_views_republican_filtered_sep[order(unlist(page_views_republican_filtered_sep), decreasing = TRUE)]

page_views_democrat_filtered_sep <- page_views_democrat_filtered_sep[order(unlist(page_views_democrat_filtered_sep), decreasing = TRUE)]

page_views_democrat_filtered_sep <- page_views_democrat_filtered_sep[!(names(page_views_democrat_filtered_sep) %in% pages_to_filter)]

page_views_republican_filtered_sep <- page_views_republican_filtered_sep[!(names(page_views_republican_filtered_sep) %in% pages_to_filter_rep)]
...
matched_names_rep_oct <- network_names_oct[network_names_oct %in% names(updated_rep_pv_list_oct)]
page_views_republican_filtered_oct <- updated_rep_pv_list_oct[matched_names_rep_oct]

matched_names_dem_oct <- network_names_oct[network_names_oct %in% names(updated_dem_pv_list_oct)]
page_views_democrat_filtered_oct <- updated_dem_pv_list_oct[matched_names_dem_oct]

# order by descending page views
page_views_republican_filtered_oct <- page_views_republican_filtered_oct[order(unlist(page_views_republican_filtered_oct), decreasing = TRUE)]

page_views_democrat_filtered_oct <- page_views_democrat_filtered_oct[order(unlist(page_views_democrat_filtered_oct), decreasing = TRUE)]

page_views_democrat_filtered_oct <- page_views_democrat_filtered_oct[!(names(page_views_democrat_filtered_oct) %in% pages_to_filter)]

page_views_republican_filtered_oct <- page_views_republican_filtered_oct[!(names(page_views_republican_filtered_oct) %in% pages_to_filter_rep)]
...
# Filter out the specified pages
page_views_democrat_filtered <- page_views_democrat_filtered[!(names(page_views_democrat_filtered) %in% pages_to_filter)]

page_views_republican_filtered <- page_views_republican_filtered[!(names(page_views_republican_filtered) %in% pages_to_filter_rep)]
...
ggplot(page_views_df, aes(x = Views)) +
  ...
  scale_x_log10() +
  theme_minimal() +
  labs(title = "Distribution of Page Views",
       x = "Page Views (log scale)",
       y = "Frequency")
```
...
page_views_democrat_filtered_dec <- page_views_democrat_filtered_dec[!(names(page_views_democrat_filtered_dec) %in% pages_to_filter)]

page_views_republican_filtered_dec <- page_views_republican_filtered_dec[!(names(page_views_republican_filtered_dec) %in% pages_to_filter_rep)]
...
common_pages_rep <- intersect(page_names_republican, page_names_republican_dec)
common_pages_dem <- intersect(page_names_democrat, page_names_democrat_dec)
```
...
matched_names_rep_jan <- network_names_jan[network_names_jan %in% names(updated_rep_pv_list_jan)]
page_views_republican_filtered_jan <- updated_rep_pv_list_jan[matched_names_rep_jan]

matched_names_dem_jan <- network_names_jan[network_names_jan %in% names(updated_dem_pv_list_jan)]
...
page_views_democrat_filtered_jan <- page_views_democrat_filtered_jan[!(names(page_views_democrat_filtered_jan) %in% pages_to_filter)]

page_views_republican_filtered_jan <- page_views_republican_filtered_jan[!(names(page_views_republican_filtered_jan) %in% pages_to_filter_rep)]
...
intersect(common_pages_rep, page_names_republican_jan)
intersect(common_pages_dem, page_names_democrat_jan)

page_views_republican_filtered_jan[1:100]
```
...
matched_names_rep_feb <- network_names_feb[network_names_feb %in% names(updated_rep_pv_list_feb)]
page_views_republican_filtered_feb <- updated_rep_pv_list_feb[matched_names_rep_feb]

matched_names_dem_feb <- network_names_feb[network_names_feb %in% names(updated_dem_pv_list_feb)]
...
page_views_democrat_filtered_feb <- page_views_democrat_filtered_feb[!(names(page_views_democrat_filtered_feb) %in% pages_to_filter)]

page_views_republican_filtered_feb <- page_views_republican_filtered_feb[!(names(page_views_republican_filtered_feb) %in% pages_to_filter_rep)]
...
intersect(common_pages_rep, page_names_republican_feb)
intersect(common_pages_dem, page_names_democrat_feb)
```
...
# Perform intersections for month 1
...
# Perform intersections for month 2
...
# Perform intersections for month 3
...
# Perform intersections for month 4
...
result <- list(November = total_count_nov,
...
result_df <- data.frame(
  Month = factor(names(result), levels = c("November", "December", "January", "February")),
  Count = unlist(result)
)

ggplot(result_df, aes(x = Month, y = Count, fill = Month)) +
  geom_bar(stat = "identity") +
  theme_minimal() +
  labs(title = "Number of Pages with Opposing Neighbors",
       x = "Month",
       y = "Count of Pages") +
...
```{r}
# get number of "Other" pages for September, October, November, December, January and February

(n_other_sep <- sum(V(net_clicksep20)$party == "Other"))
(n_other_oct <- sum(V(net_clickoct20)$party == "Other"))
(n_other_nov <- sum(V(net_clicknov20)$party == "Other"))
(n_other_dec <- sum(V(net_clickdec20)$party == "Other"))
(n_other_jan <- sum(V(net_clickjan21)$party == "Other"))
(n_other_feb <- sum(V(net_clickfeb21)$party == "Other"))
```
...
results_sep20_1d <- analyze_party_walk(
  net_clicksep20, "Democrat", page_names_democrat_sep, steps = 5, iterations = 10)

results_sep20_2d <- analyze_party_walk(
  net_clicksep20, "Democrat", page_names_democrat_sep, steps = 5, iterations = 10)

results_sep20_3d <- analyze_party_walk(
  net_clicksep20, "Democrat", page_names_democrat_sep, steps = 5, iterations = 10)

results_sep20_4d <- analyze_party_walk(
  net_clicksep20, "Democrat", page_names_democrat_sep, steps = 5, iterations = 10)

results_sep20_5d <- analyze_party_walk(
  net_clicksep20, "Democrat", page_names_democrat_sep, steps = 5, iterations = 10)

results_sep20_1r <- analyze_party_walk(
  net_clicksep20, "Republican", page_names_republican_sep, steps = 5, iterations = 10)

results_sep20_2r <- analyze_party_walk(
  net_clicksep20, "Republican", page_names_republican_sep, steps = 5, iterations = 10)

results_sep20_3r <- analyze_party_walk(
  net_clicksep20, "Republican", page_names_republican_sep, steps = 5, iterations = 10)

results_sep20_4r <- analyze_party_walk(
  net_clicksep20, "Republican", page_names_republican_sep, steps = 5, iterations = 10)

results_sep20_5r <- analyze_party_walk(
  net_clicksep20, "Republican", page_names_republican_sep, steps = 5, iterations = 10)

# aggregate the results
data_list_dem_sep <- list(
  results_sep20_1d, results_sep20_2d, results_sep20_3d, results_sep20_4d, results_sep20_5d)

data_list_rep_sep <- list(
  results_sep20_1r, results_sep20_2r, results_sep20_3r, results_sep20_4r, results_sep20_5r)

# Initialize accumulator
results_dem_sep <- list(steps = 0, n_part_shift = 0, n_trapped = 0, n_republican = 0, n_democrat = 0, n_other = 0, n_of_shift_steps_1 = 0, n_of_shift_steps_2 = 0, n_of_shift_steps_3 = 0, n_of_shift_steps_4 = 0, n_of_shift_steps_5 = 0)

steps_count <- 0

# Iterate through each object to sum and count
for (obj in data_list_dem_sep) {
  results_dem_sep$steps <- results_dem_sep$steps + obj$steps
  results_dem_sep$n_part_shift <- results_dem_sep$n_part_shift + obj$n_part_shift
  results_dem_sep$n_trapped <- results_dem_sep$n_trapped + obj$n_trapped
  results_dem_sep$n_republican <- results_dem_sep$n_republican + obj$n_republican
  results_dem_sep$n_democrat <- results_dem_sep$n_democrat + obj$n_democrat
  results_dem_sep$n_other <- results_dem_sep$n_other + obj$n_other
  results_dem_sep$n_of_shift_steps_1 <- results_dem_sep$n_of_shift_steps_1 + obj$n_of_shift_steps_1
  results_dem_sep$n_of_shift_steps_2 <- results_dem_sep$n_of_shift_steps_2 + obj$n_of_shift_steps_2
  results_dem_sep$n_of_shift_steps_3 <- results_dem_sep$n_of_shift_steps_3 + obj$n_of_shift_steps_3
  results_dem_sep$n_of_shift_steps_4 <- results_dem_sep$n_of_shift_steps_4 + obj$n_of_shift_steps_4
  results_dem_sep$n_of_shift_steps_5 <- results_dem_sep$n_of_shift_steps_5 + obj$n_of_shift_steps_5
  steps_count <- steps_count + 1
}

results_dem_sep$steps <- results_dem_sep$steps / steps_count

# initialize accumulator
results_rep_sep <- list(steps = 0, n_part_shift = 0, n_trapped = 0, n_republican = 0, n_democrat = 0, n_other = 0, n_of_shift_steps_1 = 0, n_of_shift_steps_2 = 0, n_of_shift_steps_3 = 0, n_of_shift_steps_4 = 0, n_of_shift_steps_5 = 0)

steps_count <- 0

# Iterate through each object to sum and count

for (obj in data_list_rep_sep) {
  results_rep_sep$steps <- results_rep_sep$steps + obj$steps
  results_rep_sep$n_part_shift <- results_rep_sep$n_part_shift + obj$n_part_shift
  results_rep_sep$n_trapped <- results_rep_sep$n_trapped + obj$n_trapped
  results_rep_sep$n_republican <- results_rep_sep$n_republican + obj$n_republican
  results_rep_sep$n_democrat <- results_rep_sep$n_democrat + obj$n_democrat
  results_rep_sep$n_other <- results_rep_sep$n_other + obj$n_other
  results_rep_sep$n_of_shift_steps_1 <- results_rep_sep$n_of_shift_steps_1 + obj$n_of_shift_steps_1
  results_rep_sep$n_of_shift_steps_2 <- results_rep_sep$n_of_shift_steps_2 + obj$n_of_shift_steps_2
  results_rep_sep$n_of_shift_steps_3 <- results_rep_sep$n_of_shift_steps_3 + obj$n_of_shift_steps_3
  results_rep_sep$n_of_shift_steps_4 <- results_rep_sep$n_of_shift_steps_4 + obj$n_of_shift_steps_4
  results_rep_sep$n_of_shift_steps_5 <- results_rep_sep$n_of_shift_steps_5 + obj$n_of_shift_steps_5
  steps_count <- steps_count + 1
}

results_rep_sep$steps <- results_rep_sep$steps / steps_count
```
...
results_oct20_1d <- analyze_party_walk(
  net_clickoct20, "Democrat", page_names_democrat_oct, steps = 5, iterations = 10)

results_oct20_2d <- analyze_party_walk(
  net_clickoct20, "Democrat", page_names_democrat_oct, steps = 5, iterations = 10)

results_oct20_3d <- analyze_party_walk(
  net_clickoct20, "Democrat", page_names_democrat_oct, steps = 5, iterations = 10)

results_oct20_4d <- analyze_party_walk(
  net_clickoct20, "Democrat", page_names_democrat_oct, steps = 5, iterations = 10)

results_oct20_5d <- analyze_party_walk(
  net_clickoct20, "Democrat", page_names_democrat_oct, steps = 5, iterations = 10)

results_oct20_1r <- analyze_party_walk(
  net_clickoct20, "Republican", page_names_republican_oct, steps = 5, iterations = 10)

results_oct20_2r <- analyze_party_walk(
  net_clickoct20, "Republican", page_names_republican_oct, steps = 5, iterations = 10)

results_oct20_3r <- analyze_party_walk(
  net_clickoct20, "Republican", page_names_republican_oct, steps = 5, iterations = 10)

results_oct20_4r <- analyze_party_walk(
  net_clickoct20, "Republican", page_names_republican_oct, steps = 5, iterations = 10)

results_oct20_5r <- analyze_party_walk(
  net_clickoct20, "Republican", page_names_republican_oct, steps = 5, iterations = 10)

# aggregate the results
data_list_dem_oct <- list(
  results_oct20_1d, results_oct20_2d, results_oct20_3d, results_oct20_4d, results_oct20_5d)

data_list_rep_oct <- list(
  results_oct20_1r, results_oct20_2r, results_oct20_3r, results_oct20_4r, results_oct20_5r)

# Initialize accumulator
results_dem_oct <- list(steps = 0, n_part_shift = 0, n_trapped = 0, n_republican = 0, n_democrat = 0, n_other = 0, n_of_shift_steps_1 = 0, n_of_shift_steps_2 = 0, n_of_shift_steps_3 = 0, n_of_shift_steps_4 = 0, n_of_shift_steps_5 = 0)

steps_count <- 0

# Iterate through each object to sum and count
for (obj in data_list_dem_oct) {
  results_dem_oct$steps <- results_dem_oct$steps + obj$steps
  results_dem_oct$n_part_shift <- results_dem_oct$n_part_shift + obj$n_part_shift
  results_dem_oct$n_trapped <- results_dem_oct$n_trapped + obj$n_trapped
  results_dem_oct$n_republican <- results_dem_oct$n_republican + obj$n_republican
  results_dem_oct$n_democrat <- results_dem_oct$n_democrat + obj$n_democrat
  results_dem_oct$n_other <- results_dem_oct$n_other + obj$n_other
  results_dem_oct$n_of_shift_steps_1 <- results_dem_oct$n_of_shift_steps_1 + obj$n_of_shift_steps_1
  results_dem_oct$n_of_shift_steps_2 <- results_dem_oct$n_of_shift_steps_2 + obj$n_of_shift_steps_2
  results_dem_oct$n_of_shift_steps_3 <- results_dem_oct$n_of_shift_steps_3 + obj$n_of_shift_steps_3
  results_dem_oct$n_of_shift_steps_4 <- results_dem_oct$n_of_shift_steps_4 + obj$n_of_shift_steps_4
  results_dem_oct$n_of_shift_steps_5 <- results_dem_oct$n_of_shift_steps_5 + obj$n_of_shift_steps_5
  steps_count <- steps_count + 1
}

results_dem_oct$steps <- results_dem_oct$steps / steps_count

# initialize accumulator
results_rep_oct <- list(steps = 0, n_part_shift = 0, n_trapped = 0, n_republican = 0, n_democrat = 0, n_other = 0, n_of_shift_steps_1 = 0, n_of_shift_steps_2 = 0, n_of_shift_steps_3 = 0, n_of_shift_steps_4 = 0, n_of_shift_steps_5 = 0)

steps_count <- 0

# Iterate through each object to sum and count
for (obj in data_list_rep_oct) {
  results_rep_oct$steps <- results_rep_oct$steps + obj$steps
  results_rep_oct$n_part_shift <- results_rep_oct$n_part_shift + obj$n_part_shift
  results_rep_oct$n_trapped <- results_rep_oct$n_trapped + obj$n_trapped
  results_rep_oct$n_republican <- results_rep_oct$n_republican + obj$n_republican
  results_rep_oct$n_democrat <- results_rep_oct$n_democrat + obj$n_democrat
  results_rep_oct$n_other <- results_rep_oct$n_other + obj$n_other
  results_rep_oct$n_of_shift_steps_1 <- results_rep_oct$n_of_shift_steps_1 + obj$n_of_shift_steps_1
  results_rep_oct$n_of_shift_steps_2 <- results_rep_oct$n_of_shift_steps_2 + obj$n_of_shift_steps_2
  results_rep_oct$n_of_shift_steps_3 <- results_rep_oct$n_of_shift_steps_3 + obj$n_of_shift_steps_3
  results_rep_oct$n_of_shift_steps_4 <- results_rep_oct$n_of_shift_steps_4 + obj$n_of_shift_steps_4
  results_rep_oct$n_of_shift_steps_5 <- results_rep_oct$n_of_shift_steps_5 + obj$n_of_shift_steps_5
  steps_count <- steps_count + 1
}

results_rep_oct$steps <- results_rep_oct$steps / steps_count
```


```{r rw_november}
# run random walks for november
results_nov20_1d <- analyze_party_walk(net_clicknov20, "Democrat", page_names_democrat, steps = 5, iterations = 10)

results_nov20_2d <- analyze_party_walk(net_clicknov20, "Democrat", page_names_democrat, steps = 5, iterations = 10)

results_nov20_3d <- analyze_party_walk(net_clicknov20, "Democrat", page_names_democrat, steps = 5, iterations = 10)

results_nov20_4d <- analyze_party_walk(net_clicknov20, "Democrat", page_names_democrat, steps = 5, iterations = 10)

results_nov20_5d <- analyze_party_walk(net_clicknov20, "Democrat", page_names_democrat, steps = 5, iterations = 10)

results_nov20_1r <- analyze_party_walk(net_clicknov20, "Republican", page_names_republican, steps = 5, iterations = 10)

results_nov20_2r <- analyze_party_walk(net_clicknov20, "Republican", page_names_republican, steps = 5, iterations = 10)

results_nov20_3r <- analyze_party_walk(net_clicknov20, "Republican", page_names_republican, steps = 5, iterations = 10)

results_nov20_4r <- analyze_party_walk(net_clicknov20, "Republican", page_names_republican, steps = 5, iterations = 10)

results_nov20_5r <- analyze_party_walk(net_clicknov20, "Republican", page_names_republican, steps = 5, iterations = 10)

# aggregate the results
data_list_dem_nov <- list(
  results_nov20_1d, results_nov20_2d, results_nov20_3d, results_nov20_4d, results_nov20_5d)

data_list_rep_nov <- list(
  results_nov20_1r, results_nov20_2r, results_nov20_3r, results_nov20_4r, results_nov20_5r)

# Initialize accumulator
results_dem_nov <- list(steps = 0, n_part_shift = 0, n_trapped = 0, n_republican = 0, n_democrat = 0, n_other = 0, n_of_shift_steps_1 = 0, n_of_shift_steps_2 = 0, n_of_shift_steps_3 = 0, n_of_shift_steps_4 = 0, n_of_shift_steps_5 = 0)

steps_count <- 0

# Iterate through each object to sum and count
for (obj in data_list_dem_nov) {
  results_dem_nov$steps <- results_dem_nov$steps + obj$steps
  results_dem_nov$n_part_shift <- results_dem_nov$n_part_shift + obj$n_part_shift
  results_dem_nov$n_trapped <- results_dem_nov$n_trapped + obj$n_trapped
  results_dem_nov$n_republican <- results_dem_nov$n_republican + obj$n_republican
  results_dem_nov$n_democrat <- results_dem_nov$n_democrat + obj$n_democrat
  results_dem_nov$n_other <- results_dem_nov$n_other + obj$n_other
  results_dem_nov$n_of_shift_steps_1 <- results_dem_nov$n_of_shift_steps_1 + obj$n_of_shift_steps_1
  results_dem_nov$n_of_shift_steps_2 <- results_dem_nov$n_of_shift_steps_2 + obj$n_of_shift_steps_2
  results_dem_nov$n_of_shift_steps_3 <- results_dem_nov$n_of_shift_steps_3 + obj$n_of_shift_steps_3
  results_dem_nov$n_of_shift_steps_4 <- results_dem_nov$n_of_shift_steps_4 + obj$n_of_shift_steps_4
  results_dem_nov$n_of_shift_steps_5 <- results_dem_nov$n_of_shift_steps_5 + obj$n_of_shift_steps_5
  steps_count <- steps_count + 1
}

results_dem_nov$steps <- results_dem_nov$steps / steps_count

# initialize accumulator
results_rep_nov <- list(steps = 0, n_part_shift = 0, n_trapped = 0, n_republican = 0, n_democrat = 0, n_other = 0, n_of_shift_steps_1 = 0, n_of_shift_steps_2 = 0, n_of_shift_steps_3 = 0, n_of_shift_steps_4 = 0, n_of_shift_steps_5 = 0)

steps_count <- 0

# Iterate through each object to sum and count
for (obj in data_list_rep_nov) {
  results_rep_nov$steps <- results_rep_nov$steps + obj$steps
  results_rep_nov$n_part_shift <- results_rep_nov$n_part_shift + obj$n_part_shift
  results_rep_nov$n_trapped <- results_rep_nov$n_trapped + obj$n_trapped
  results_rep_nov$n_republican <- results_rep_nov$n_republican + obj$n_republican
  results_rep_nov$n_democrat <- results_rep_nov$n_democrat + obj$n_democrat
  results_rep_nov$n_other <- results_rep_nov$n_other + obj$n_other
  results_rep_nov$n_of_shift_steps_1 <- results_rep_nov$n_of_shift_steps_1 + obj$n_of_shift_steps_1
  results_rep_nov$n_of_shift_steps_2 <- results_rep_nov$n_of_shift_steps_2 + obj$n_of_shift_steps_2
  results_rep_nov$n_of_shift_steps_3 <- results_rep_nov$n_of_shift_steps_3 + obj$n_of_shift_steps_3
  results_rep_nov$n_of_shift_steps_4 <- results_rep_nov$n_of_shift_steps_4 + obj$n_of_shift_steps_4
  results_rep_nov$n_of_shift_steps_5 <- results_rep_nov$n_of_shift_steps_5 + obj$n_of_shift_steps_5
  steps_count <- steps_count + 1
}
```
...
results_dec20_1d <- analyze_party_walk(net_clickdec20, "Democrat", page_names_democrat_dec, steps = 5, iterations = 10)

results_dec20_2d <- analyze_party_walk(net_clickdec20, "Democrat", page_names_democrat_dec, steps = 5, iterations = 10)

results_dec20_3d <- analyze_party_walk(net_clickdec20, "Democrat", page_names_democrat_dec, steps = 5, iterations = 10)

results_dec20_4d <- analyze_party_walk(net_clickdec20, "Democrat", page_names_democrat_dec, steps = 5, iterations = 10)

results_dec20_5d <- analyze_party_walk(net_clickdec20, "Democrat", page_names_democrat_dec, steps = 5, iterations = 10)

results_dec20_1r <- analyze_party_walk(net_clickdec20, "Republican", page_names_republican_dec, steps = 5, iterations = 10)

results_dec20_2r <- analyze_party_walk(net_clickdec20, "Republican", page_names_republican_dec, steps = 5, iterations = 10)

results_dec20_3r <- analyze_party_walk(net_clickdec20, "Republican", page_names_republican_dec, steps = 5, iterations = 10)

results_dec20_4r <- analyze_party_walk(net_clickdec20, "Republican", page_names_republican_dec, steps = 5, iterations = 10)

results_dec20_5r <- analyze_party_walk(net_clickdec20, "Republican", page_names_republican_dec, steps = 5, iterations = 10)

# aggregate the results
data_list_dem_dec <- list(
  results_dec20_1d, results_dec20_2d, results_dec20_3d, results_dec20_4d, results_dec20_5d)

data_list_rep_dec <- list(
  results_dec20_1r, results_dec20_2r, results_dec20_3r, results_dec20_4r, results_dec20_5r)

# Initialize accumulator
results_dem_dec <- list(steps = 0, n_part_shift = 0, n_trapped = 0, n_republican = 0, n_democrat = 0, n_other = 0, n_of_shift_steps_1 = 0, n_of_shift_steps_2 = 0, n_of_shift_steps_3 = 0, n_of_shift_steps_4 = 0, n_of_shift_steps_5 = 0)

steps_count <- 0

# Iterate through each object to sum and count
for (obj in data_list_dem_dec) {
  results_dem_dec$steps <- results_dem_dec$steps + obj$steps
  results_dem_dec$n_part_shift <- results_dem_dec$n_part_shift + obj$n_part_shift
  results_dem_dec$n_trapped <- results_dem_dec$n_trapped + obj$n_trapped
  results_dem_dec$n_republican <- results_dem_dec$n_republican + obj$n_republican
  results_dem_dec$n_democrat <- results_dem_dec$n_democrat + obj$n_democrat
  results_dem_dec$n_other <- results_dem_dec$n_other + obj$n_other
  results_dem_dec$n_of_shift_steps_1 <- results_dem_dec$n_of_shift_steps_1 + obj$n_of_shift_steps_1
  results_dem_dec$n_of_shift_steps_2 <- results_dem_dec$n_of_shift_steps_2 + obj$n_of_shift_steps_2
  results_dem_dec$n_of_shift_steps_3 <- results_dem_dec$n_of_shift_steps_3 + obj$n_of_shift_steps_3
  results_dem_dec$n_of_shift_steps_4 <- results_dem_dec$n_of_shift_steps_4 + obj$n_of_shift_steps_4
  results_dem_dec$n_of_shift_steps_5 <- results_dem_dec$n_of_shift_steps_5 + obj$n_of_shift_steps_5
  steps_count <- steps_count + 1
}

results_dem_dec$steps <- results_dem_dec$steps / steps_count

# initialize accumulator
results_rep_dec <- list(steps = 0, n_part_shift = 0, n_trapped = 0, n_republican = 0, n_democrat = 0, n_other = 0, n_of_shift_steps_1 = 0, n_of_shift_steps_2 = 0, n_of_shift_steps_3 = 0, n_of_shift_steps_4 = 0, n_of_shift_steps_5 = 0)

steps_count <- 0

# Iterate through each object to sum and count
for (obj in data_list_rep_dec) {
  results_rep_dec$steps <- results_rep_dec$steps + obj$steps
  results_rep_dec$n_part_shift <- results_rep_dec$n_part_shift + obj$n_part_shift
  results_rep_dec$n_trapped <- results_rep_dec$n_trapped + obj$n_trapped
  results_rep_dec$n_republican <- results_rep_dec$n_republican + obj$n_republican
  results_rep_dec$n_democrat <- results_rep_dec$n_democrat + obj$n_democrat
  results_rep_dec$n_other <- results_rep_dec$n_other + obj$n_other
  results_rep_dec$n_of_shift_steps_1 <- results_rep_dec$n_of_shift_steps_1 + obj$n_of_shift_steps_1
  results_rep_dec$n_of_shift_steps_2 <- results_rep_dec$n_of_shift_steps_2 + obj$n_of_shift_steps_2
  results_rep_dec$n_of_shift_steps_3 <- results_rep_dec$n_of_shift_steps_3 + obj$n_of_shift_steps_3
  results_rep_dec$n_of_shift_steps_4 <- results_rep_dec$n_of_shift_steps_4 + obj$n_of_shift_steps_4
  results_rep_dec$n_of_shift_steps_5 <- results_rep_dec$n_of_shift_steps_5 + obj$n_of_shift_steps_5
  steps_count <- steps_count + 1
}

results_rep_dec$steps <- results_rep_dec$steps / steps_count
```
...
results_jan21_1d <- analyze_party_walk(net_clickjan21, "Democrat", page_names_democrat_jan, steps = 5, iterations = 10)

results_jan21_2d <- analyze_party_walk(net_clickjan21, "Democrat", page_names_democrat_jan, steps = 5, iterations = 10)

results_jan21_3d <- analyze_party_walk(net_clickjan21, "Democrat", page_names_democrat_jan, steps = 5, iterations = 10)

results_jan21_4d <- analyze_party_walk(net_clickjan21, "Democrat", page_names_democrat_jan, steps = 5, iterations = 10)

results_jan21_5d <- analyze_party_walk(net_clickjan21, "Democrat", page_names_democrat_jan, steps = 5, iterations = 10)

results_jan21_1r <- analyze_party_walk(net_clickjan21, "Republican", page_names_republican_jan, steps = 5, iterations = 10)

results_jan21_2r <- analyze_party_walk(net_clickjan21, "Republican", page_names_republican_jan, steps = 5, iterations = 10)

results_jan21_3r <- analyze_party_walk(net_clickjan21, "Republican", page_names_republican_jan, steps = 5, iterations = 10)

results_jan21_4r <- analyze_party_walk(net_clickjan21, "Republican", page_names_republican_jan, steps = 5, iterations = 10)

results_jan21_5r <- analyze_party_walk(net_clickjan21, "Republican", page_names_republican_jan, steps = 5, iterations = 10)

# aggregate the results
data_list_dem_jan <- list(
  results_jan21_1d, results_jan21_2d, results_jan21_3d, results_jan21_4d, results_jan21_5d)

data_list_rep_jan <- list(
  results_jan21_1r, results_jan21_2r, results_jan21_3r, results_jan21_4r, results_jan21_5r)

# Initialize accumulator

results_dem_jan <- list(steps = 0, n_part_shift = 0, n_trapped = 0, n_republican = 0, n_democrat = 0, n_other = 0, n_of_shift_steps_1 = 0, n_of_shift_steps_2 = 0, n_of_shift_steps_3 = 0, n_of_shift_steps_4 = 0, n_of_shift_steps_5 = 0)

steps_count <- 0

# Iterate through each object to sum and count
for (obj in data_list_dem_jan) {
  results_dem_jan$steps <- results_dem_jan$steps + obj$steps
  results_dem_jan$n_part_shift <- results_dem_jan$n_part_shift + obj$n_part_shift
  results_dem_jan$n_trapped <- results_dem_jan$n_trapped + obj$n_trapped
  results_dem_jan$n_republican <- results_dem_jan$n_republican + obj$n_republican
  results_dem_jan$n_democrat <- results_dem_jan$n_democrat + obj$n_democrat
  results_dem_jan$n_other <- results_dem_jan$n_other + obj$n_other
  results_dem_jan$n_of_shift_steps_1 <- results_dem_jan$n_of_shift_steps_1 + obj$n_of_shift_steps_1
  results_dem_jan$n_of_shift_steps_2 <- results_dem_jan$n_of_shift_steps_2 + obj$n_of_shift_steps_2
  results_dem_jan$n_of_shift_steps_3 <- results_dem_jan$n_of_shift_steps_3 + obj$n_of_shift_steps_3
  results_dem_jan$n_of_shift_steps_4 <- results_dem_jan$n_of_shift_steps_4 + obj$n_of_shift_steps_4
  results_dem_jan$n_of_shift_steps_5 <- results_dem_jan$n_of_shift_steps_5 + obj$n_of_shift_steps_5
  steps_count <- steps_count + 1
}

# Calculate the average of steps
results_dem_jan$steps <- results_dem_jan$steps / steps_count

# initialize accumulator
results_rep_jan <- list(steps = 0, n_part_shift = 0, n_trapped = 0, n_republican = 0, n_democrat = 0, n_other = 0, n_of_shift_steps_1 = 0, n_of_shift_steps_2 = 0, n_of_shift_steps_3 = 0, n_of_shift_steps_4 = 0, n_of_shift_steps_5 = 0)

steps_count <- 0

# Iterate through each object to sum and count
for (obj in data_list_rep_jan) {
  results_rep_jan$steps <- results_rep_jan$steps + obj$steps
  results_rep_jan$n_part_shift <- results_rep_jan$n_part_shift + obj$n_part_shift
  results_rep_jan$n_trapped <- results_rep_jan$n_trapped + obj$n_trapped
  results_rep_jan$n_republican <- results_rep_jan$n_republican + obj$n_republican
  results_rep_jan$n_democrat <- results_rep_jan$n_democrat + obj$n_democrat
  results_rep_jan$n_other <- results_rep_jan$n_other + obj$n_other
  results_rep_jan$n_of_shift_steps_1 <- results_rep_jan$n_of_shift_steps_1 + obj$n_of_shift_steps_1
  results_rep_jan$n_of_shift_steps_2 <- results_rep_jan$n_of_shift_steps_2 + obj$n_of_shift_steps_2
  results_rep_jan$n_of_shift_steps_3 <- results_rep_jan$n_of_shift_steps_3 + obj$n_of_shift_steps_3
  results_rep_jan$n_of_shift_steps_4 <- results_rep_jan$n_of_shift_steps_4 + obj$n_of_shift_steps_4
  results_rep_jan$n_of_shift_steps_5 <- results_rep_jan$n_of_shift_steps_5 + obj$n_of_shift_steps_5
  steps_count <- steps_count + 1
}

# Calculate the average of steps
results_rep_jan$steps <- results_rep_jan$steps / steps_count
```


```{r rw_february}
# run random walks for february
results_feb21_1d <- analyze_party_walk(net_clickfeb21, "Democrat", page_names_democrat_feb, steps = 5, iterations = 10)

results_feb21_2d <- analyze_party_walk(net_clickfeb21, "Democrat", page_names_democrat_feb, steps = 5, iterations = 10)

results_feb21_3d <- analyze_party_walk(net_clickfeb21, "Democrat", page_names_democrat_feb, steps = 5, iterations = 10)

results_feb21_4d <- analyze_party_walk(net_clickfeb21, "Democrat", page_names_democrat_feb, steps = 5, iterations = 10)

results_feb21_5d <- analyze_party_walk(net_clickfeb21, "Democrat", page_names_democrat_feb, steps = 5, iterations = 10)

results_feb21_1r <- analyze_party_walk(net_clickfeb21, "Republican", page_names_republican_feb, steps = 5, iterations = 10)

results_feb21_2r <- analyze_party_walk(net_clickfeb21, "Republican", page_names_republican_feb, steps = 5, iterations = 10)

results_feb21_3r <- analyze_party_walk(net_clickfeb21, "Republican", page_names_republican_feb, steps = 5, iterations = 10)

results_feb21_4r <- analyze_party_walk(net_clickfeb21, "Republican", page_names_republican_feb, steps = 5, iterations = 10)

results_feb21_5r <- analyze_party_walk(net_clickfeb21, "Republican", page_names_republican_feb, steps = 5, iterations = 10)

# aggregate the results
data_list_dem_feb <- list(
  results_feb21_1d, results_feb21_2d, results_feb21_3d, results_feb21_4d, results_feb21_5d)

data_list_rep_feb <- list(
  results_feb21_1r, results_feb21_2r, results_feb21_3r, results_feb21_4r, results_feb21_5r)

# Initialize accumulator
results_dem_feb <- list(steps = 0, n_part_shift = 0, n_trapped = 0, n_republican = 0, n_democrat = 0, n_other = 0, n_of_shift_steps_1 = 0, n_of_shift_steps_2 = 0, n_of_shift_steps_3 = 0, n_of_shift_steps_4 = 0, n_of_shift_steps_5 = 0)
steps_count <- 0

# Iterate through each object to sum and count
for (obj in data_list_dem_feb) {
  results_dem_feb$steps <- results_dem_feb$steps + obj$steps
  results_dem_feb$n_part_shift <- results_dem_feb$n_part_shift + obj$n_part_shift
  results_dem_feb$n_trapped <- results_dem_feb$n_trapped + obj$n_trapped
  results_dem_feb$n_republican <- results_dem_feb$n_republican + obj$n_republican
  results_dem_feb$n_democrat <- results_dem_feb$n_democrat + obj$n_democrat
  results_dem_feb$n_other <- results_dem_feb$n_other + obj$n_other
  results_dem_feb$n_of_shift_steps_1 <- results_dem_feb$n_of_shift_steps_1 + obj$n_of_shift_steps_1
  results_dem_feb$n_of_shift_steps_2 <- results_dem_feb$n_of_shift_steps_2 + obj$n_of_shift_steps_2
  results_dem_feb$n_of_shift_steps_3 <- results_dem_feb$n_of_shift_steps_3 + obj$n_of_shift_steps_3
  results_dem_feb$n_of_shift_steps_4 <- results_dem_feb$n_of_shift_steps_4 + obj$n_of_shift_steps_4
  results_dem_feb$n_of_shift_steps_5 <- results_dem_feb$n_of_shift_steps_5 + obj$n_of_shift_steps_5
  steps_count <- steps_count + 1
}

# Calculate the average of steps
results_dem_feb$steps <- results_dem_feb$steps / steps_count

# Initialize accumulator
results_rep_feb <- list(steps = 0, n_part_shift = 0, n_trapped = 0, n_republican = 0, n_democrat = 0, n_other = 0, n_of_shift_steps_1 = 0, n_of_shift_steps_2 = 0, n_of_shift_steps_3 = 0, n_of_shift_steps_4 = 0, n_of_shift_steps_5 = 0)
steps_count <- 0

# Iterate through each object to sum and count
for (obj in data_list_rep_feb) {
  results_rep_feb$steps <- results_rep_feb$steps + obj$steps
  results_rep_feb$n_part_shift <- results_rep_feb$n_part_shift + obj$n_part_shift
  results_rep_feb$n_trapped <- results_rep_feb$n_trapped + obj$n_trapped
  results_rep_feb$n_republican <- results_rep_feb$n_republican + obj$n_republican
  results_rep_feb$n_democrat <- results_rep_feb$n_democrat + obj$n_democrat
  results_rep_feb$n_other <- results_rep_feb$n_other + obj$n_other
  results_rep_feb$n_of_shift_steps_1 <- results_rep_feb$n_of_shift_steps_1 + obj$n_of_shift_steps_1
  results_rep_feb$n_of_shift_steps_2 <- results_rep_feb$n_of_shift_steps_2 + obj$n_of_shift_steps_2
  results_rep_feb$n_of_shift_steps_3 <- results_rep_feb$n_of_shift_steps_3 + obj$n_of_shift_steps_3
  results_rep_feb$n_of_shift_steps_4 <- results_rep_feb$n_of_shift_steps_4 + obj$n_of_shift_steps_4
  results_rep_feb$n_of_shift_steps_5 <- results_rep_feb$n_of_shift_steps_5 + obj$n_of_shift_steps_5
  steps_count <- steps_count + 1
}

# Calculate the average of steps
results_rep_feb$steps <- results_rep_feb$steps / steps_count
```
...
# Display the table
rw_table <- kable(results, caption = "<span style='color: black; font-weight: bold'>Table 2. Results of Random Walks by Month</span>")

rw_table <- kable(results, 
                   caption = "<div style='text-align: center;'><span style='color: black; font-weight: bold;'>Table 2. Results of Random Walks by Month</span></div>",
                   col.names = c("Month", "Average Steps (Democrat)", "Average Steps (Republican)", "N of Shifts (Democrat)", "N of Shifts (Republican)")) %>%
  kable_styling(full_width = FALSE, 
                font_size = 12,
                html_font = "Times New Roman") %>%
  add_header_above(c(" " = 1, "Results" = 4)) %>%
  row_spec(0, bold = TRUE, font_size = 14, color = "black", background = "#f2f2f2") %>%
  row_spec(1:nrow(results), font_size = 12)
...
random_net_sep20_full <- sample_degseq(out.deg = degree(net_clicksep20, mode='out'), in.deg = degree(net_clicksep20, mode='in'), method = "simple")

random_net_oct20_full <- sample_degseq(out.deg = degree(net_clickoct20, mode='out'), in.deg = degree(net_clickoct20, mode='in'), method = "simple")

random_net_nov20_full <- sample_degseq(out.deg = degree(net_clicknov20, mode='out'), in.deg = degree(net_clicknov20, mode='in'), method = "simple")

random_net_dec20_full <- sample_degseq(out.deg = degree(net_clickdec20, mode='out'), in.deg = degree(net_clickdec20, mode='in'), method = "simple")

random_net_jan21_full <- sample_degseq(out.deg = degree(net_clickjan21, mode='out'), in.deg = degree(net_clickjan21, mode='in'), method = "simple")

random_net_feb21_full <- sample_degseq(out.deg = degree(net_clickfeb21, mode='out'), in.deg = degree(net_clickfeb21, mode='in'), method = "simple")
...
random_walk_random_networks <- function(network) {
  ...
  # Perform the analyze_party_walk for Democratic nodes
  ...
  # Return the results of the random walk
  return(
...
calculate_exposure <- function(results) {
  ...
  # Calculate the exposure to diverse information
...
barplot <- ggplot(exposure_data_long, aes(x = month, y = value, fill = variable)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(
    title = "Figure 4. Exposure to Diverse Information by Party",
    x = "Month",
  ...
  theme(
    ...
    plot.title = element_text(hjust = 0.5, size = 18),  # Center the plot title and set size
    axis.title = element_text(size = 15),  # Set size for axis titles
    axis.text = element_text(size = 10),  # Set size for axis text
    legend.text = element_text(size = 10, color = "black"),  # Set size and color for legend text
    legend.title = element_text(size = 15, color = "black")  # Set size and color for legend title
  )
```

```{r}
# Define the function to calculate exposure percentage
calculate_exposure_percentage <- function(results) {
  total_walks <- 5000
  shift_steps <- c(results$n_of_shift_steps_1, results$n_of_shift_steps_2, results$n_of_shift_steps_3, results$n_of_shift_steps_4, results$n_of_shift_steps_5)
...
exposure_df <- bind_rows(
  combine_and_average_steps("September 2020", exposure_percentages_sep$democrat, exposure_percentages_sep$republican),
  combine_and_average_steps("October 2020", exposure_percentages_oct$democrat, exposure_percentages_oct$republican),
  combine_and_average_steps("November 2020", exposure_percentages_nov$democrat, exposure_percentages_nov$republican),
  combine_and_average_steps("December 2020", exposure_percentages_dec$democrat, exposure_percentages_dec$republican),
  combine_and_average_steps("January 2021", exposure_percentages_jan$democrat, exposure_percentages_jan$republican),
  combine_and_average_steps("February 2021", exposure_percentages_feb$democrat, exposure_percentages_feb$republican)
)
...
ggplot(exposure_df, aes(x = steps, y = exposure, color = month, group = month)) +
  ...
  labs(
    title = "Figure 5. Exposure to Diverse Information By Number of Steps",
    x = "Steps",
...
```{r}
# calculate exposure for results of random networks 
...
ggplot(exposure_random_data_long, aes(x = month, y = value, fill = variable)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Exposure to Diverse Information by Party in Random Networks",
       x = "Month",
       y = "Exposure to Diverse Information",
       fill = "Party") +
  theme_minimal()
```
```{r}
# Define the function to calculate exposure percentage for the results of random networks
calculate_exposure <- function(results) {
  total_walks <- 5000
  shift_steps <- c(results$n_of_shift_steps_1, results$n_of_shift_steps_2, results$n_of_shift_steps_3, results$n_of_shift_steps_4, results$n_of_shift_steps_5)
...
# Calculate exposure percentages for random networks
exposure_random_sep20 <- calculate_exposure(results_random_sep20_dem)
exposure_random_sep20_rep <- calculate_exposure(results_random_sep20_rep)
exposure_random_oct20 <- calculate_exposure(results_random_oct20_dem)
exposure_random_oct20_rep <- calculate_exposure(results_random_oct20_rep)
...
# Ensure that the variable column is a factor with correct levels
...
barplot_random <- ggplot(exposure_random_data_long, aes(x = month, y = value, fill = variable)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(
    title = "Figure 5. Exposure to Diverse Information by Party in Random Networks",
    x = "Month",
    y = "Exposure to Diverse Information",
    fill = "Party"
  ) +
  ...
  theme(
    text = element_text(family = "Times New Roman"),  # Set all text to Times New Roman
    plot.title = element_text(hjust = 0.5, size = 15),  # Center the plot title and set size
    axis.title = element_text(size = 12),  # Set size for axis titles
    axis.text = element_text(size = 8),  # Set size for axis text
    legend.text = element_text(size = 8, color = "black"),  # Set size and color for legend text
    legend.title = element_text(size = 12, color = "black")  # Set size and color for legend title
  )
...
combined_plot <- (barplot + barplot_random) +
  plot_layout(ncol = 2, widths = c(0.5, 0.5))  # Adjust widths as needed
...
```{r}
# Calculate exposure percentage for results of random networks
calculate_exposure_percentage <- function(results) {
  total_walks <- 5000
  shift_steps <- c(results$n_of_shift_steps_1, results$n_of_shift_steps_2, results$n_of_shift_steps_3, results$n_of_shift_steps_4, results$n_of_shift_steps_5)
...
exposure_random_df <- bind_rows(
  combine_and_average_steps("September 2020", exposure_percentages_random_sep$democrat, exposure_percentages_random_sep$republican),
  combine_and_average_steps("October 2020", exposure_percentages_random_oct$democrat, exposure_percentages_random_oct$republican),
  combine_and_average_steps("November 2020", exposure_percentages_random_nov$democrat, exposure_percentages_random_nov$republican),
  combine_and_average_steps("December 2020", exposure_percentages_random_dec$democrat, exposure_percentages_random_dec$republican),
  combine_and_average_steps("January 2021", exposure_percentages_random_jan$democrat, exposure_percentages_random_jan$republican),
  combine_and_average_steps("February 2021", exposure_percentages_random_feb$democrat, exposure_percentages_random_feb$republican)
)
...
create_net_robust <- function(full_network) {
  # Identify Republican nodes
  republican_nodes <- V(full_network)[party == "Republican"]
  
  # Calculate weighted degrees for Republican nodes
  republican_degrees <- strength(full_network, vids = republican_nodes, weights = E(full_network)$weight)
  
  # Sort Republican nodes by weighted degree in ascending order
  sorted_republican_nodes <- republican_nodes[order(republican_degrees)]
  
  # Identify the 200 Republican nodes with the smallest weighted degree
  nodes_to_remove <- head(sorted_republican_nodes, 200)
...
# Create a list of mixing matrices
...
# Extract counts from the mixing matrices
extract_counts <- function(matrix, month) {
  data.frame(
    Month = month,
    "Democrat to Democrat" = matrix["Democrat", "Democrat"],
...
modularity_scores <- data.frame(
  ...
  Modularity = c(
    calculate_modularity(robust_net_nov19), calculate_modularity(robust_net_dec19), calculate_modularity(robust_net_jan20),
    calculate_modularity(robust_net_feb20), calculate_modularity(robust_net_mar20), calculate_modularity(robust_net_apr20),
    calculate_modularity(robust_net_may20), calculate_modularity(robust_net_jun20), calculate_modularity(robust_net_jul20),
    calculate_modularity(robust_net_aug20), calculate_modularity(robust_net_sep20), calculate_modularity(robust_net_oct20),
    calculate_modularity(robust_net_nov20), calculate_modularity(robust_net_dec20), calculate_modularity(robust_net_jan21),
    calculate_modularity(robust_net_feb21), calculate_modularity(robust_net_mar21), calculate_modularity(robust_net_apr21),
    calculate_modularity(robust_net_may21), calculate_modularity(robust_net_jun21), calculate_modularity(robust_net_jul21),
    calculate_modularity(robust_net_aug21), calculate_modularity(robust_net_sep21), calculate_modularity(robust_net_oct21),

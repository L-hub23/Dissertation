...
create_net_robust <- function(full_network) {
 ...
 # all_degrees <- strength(full_network, weights = E(full_network)$weight)
  
  # Identify the top 100 nodes with the highest weighted degree
#  top_100_nodes <- names(sort(all_degrees, decreasing = TRUE)[1:100])
  
  # Identify all nodes
#  all_nodes <- names(all_degrees)
  
  # Randomly select 200 nodes to remove, excluding the top 100 nodes
 # nodes_to_exclude <- intersect(top_100_nodes, all_nodes)
 # remaining_nodes <- setdiff(all_nodes, nodes_to_exclude)
...
average_modularit_robust <- mean(modularity_scores$Modularity, na.rm = TRUE)
...
# Plot the exposure to diverse information per step
